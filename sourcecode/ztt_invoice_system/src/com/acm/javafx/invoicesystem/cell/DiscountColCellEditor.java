...
public class DiscountColCellEditor extends TableCell<OthersInvoiceItems, Object>{
	@Override
	protected void updateItem(Object item, boolean empty) {
		super.updateItem(item, empty);
		if(getItem()!=null&&!empty) {
			OthersInvoiceDaoImpl.setIndex(getIndex());
			if(OthersInvoiceDaoImpl.getInvoiceItem().getDiscountType()=="Percentage") {
				setText(getString().concat("%"));
			}
			else if(OthersInvoiceDaoImpl.getInvoiceItem().isFOC()) {
				setText("FOC");
			}
			else {
				setText(getString());
